    ...
    try {
      ...
      const updatedTurtle = await prisma.turtles.update({
        ...
        data: {
          ...
          Weapons: newData?.weaponId
            ? { connect: { id: newData.weaponId } }
            : undefined,
            Pizzas_Turtles_favoritePizzaIdToPizzas: newData.favoritePizzaId
            ? { connect: { id: newData.favoritePizzaId } }
            : undefined,
            Pizzas_Turtles_secondFavoritePizzaIdToPizzas: newData.secondFavoritePizzaId
  ...
  async bindFavoritePizzaTurtle(turtleId, pizzaId) {
    try {
      ...
      await prisma.turtles.update({
        where: { id: parseInt(turtleId) },
        data: {
          Pizzas_Turtles_favoritePizzaIdToPizzas: { connect: { id: pizzaId } },
        },
      });
  ...
  async bindSecondFavoritePizzaTurtle(turtleId, pizzaId) {
    try {
      ...
      await prisma.turtles.update({
        where: { id: parseInt(turtleId) },
        data: {
          Pizzas_Turtles_secondFavoritePizzaIdToPizzas: { connect: { id: pizzaId } },
        },
      });
