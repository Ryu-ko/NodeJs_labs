...
io.on("connection", (socket) => {
    ...
    socket.on("joinRoom", ({ username }) => {
        ...
        socket.broadcast.emit("message", { user: "Server", text: `${username} has joined the chat`,time:curtime });
    });
    ...
    socket.on("message", (message) => {
        ...
        io.emit("message", { user: socket.username, text: message, time:curtime });
    });

    socket.on("leaveRoom", () => {
        io.emit("message", { user: "Server", text: `${socket.username} has left the chat`,time:curtime  });
    });
});
